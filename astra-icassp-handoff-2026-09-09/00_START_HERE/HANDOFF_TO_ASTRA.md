...
- the expanded files under `01_PRIMARY_WORKING_BASE/source/`
...
Use `00_START_HERE/main_rebalanced_vs_latest_typography.diff` only as a textual/scientific delta reference for the later rejected redraw. Do NOT blindly reuse the rejected Figure 1/2/3 visual changes.

## Figure assets Astra should use as raw material
Prefer original/pre-redraw assets under `04_ORIGINAL_FIGURE_ASSETS/` and the previous GPT-6-Ultra reference material there. The user wants Astra to redesign the figures rather than inherit the rejected latest ChatGPT redraw.
...
- Page 5 should be references only; 10-15 references is the target (currently 15).
...
2. Full compilable LaTeX source folder.
...
## Repository note
This GitHub handoff is intentionally expanded rather than uploaded as ZIP archives so Astra can browse files directly in the web UI. Large binary duplicates are omitted when the same information is represented by source/vector/reference material.
